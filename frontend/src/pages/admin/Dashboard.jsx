...
const Dashboard = () => {
    ...
    return (
        ...
        >
            <header>
                <h1 className="text-4xl font-display font-bold">Admin <span className="text-primary italic">Dashboard</span></h1>
                <p className="text-slate-500">Manage your shop operations and track performance.</p>
            </header>

            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-8">
                {cards.map((card, idx) => (
                    <div key={idx} className="p-8 bg-dark-card border border-white/5 rounded-3xl space-y-6 group hover:border-primary/30 transition-all shadow-xl">
                        <div className={`w-14 h-14 rounded-2xl ${card.bg} ${card.color} flex items-center justify-center transition-all group-hover:scale-110`}>
                            <card.icon size={28} />
                        </div>
                        <div className="space-y-1">
                            <p className="text-xs font-bold text-slate-500 uppercase tracking-widest">{card.title}</p>
                            <h4 className="text-3xl font-bold text-white tracking-tight">{card.value}</h4>
                        </div>
                    </div>
                ))}
            </div>
            ...
            {/* Recent Orders List */}
            <section className="bg-dark-card border border-white/5 rounded-[2rem] overflow-hidden">
                <div className="p-8 border-b border-white/5 flex justify-between items-center bg-white/3">
                    <h3 className="text-xl font-display font-bold">Recent Bookings</h3>
                    <button onClick={() => navigate('/admin/orders')} className="text-primary hover:text-white transition-colors text-sm font-bold">View All</button>
                </div>
                        ...
                        <table className="w-full text-left border-collapse min-w-[600px]">
                            <thead>
                                <tr className="border-b border-white/5 bg-white/5 text-xs uppercase tracking-wider text-slate-400">
                                    <th className="p-4 pl-8 font-medium">Order ID</th>
                            ...
                            <tbody className="divide-y divide-white/5">
                                {recentOrders.map((order) => (
                                    <tr key={order?._id || Math.random()} className="hover:bg-white/5 transition-colors">
                                        <td className="p-4 pl-8 text-sm font-mono text-slate-300">#{order?._id?.slice(-8).toUpperCase()}</td>
                                        <td className="p-4 text-sm text-slate-400">{order?.createdAt ? new Date(order.createdAt).toLocaleDateString() : 'N/A'}</td>
                                        <td className="p-4 text-sm font-bold text-white">₹{order?.totalPrice || 0}</td>
                                        <td className="p-4">
                                            {order?.status === 'delivered' ? (
                                                <span className="text-green-500 font-bold text-xs uppercase">Successful</span>
                                            ) : order?.status === 'cancelled' ? (
                                                <span className="text-red-500 font-bold text-xs uppercase">Cancelled</span>
                                            ) : (
